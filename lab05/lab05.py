...
def delete(t, target):
    ...
    kpop_king
      korean
        ...
        wedding dress
    """
    if target in [root(i) for i in branches(t)]:
    	return tree(root(t), [i for i in branches(t) if root(i) != target])
    else:
    	return tree(root(t), [delete(b, target) for b in branches(t)])
