...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    x = [distance(location,i) for i in centroids]
    return centroids[x.index(min(x))]
    # END Question 3
...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    closest_centroid = [find_closest(restaurant_location(i),centroids) for i in restaurants]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    # BEGIN Question 7
    xx = [i - mean(xs) for i in xs]
    yy = [i - mean(ys) for i in ys]
    xy = zip(xx,yy)
    xy = [i[0] * i[1] for i in xy]
    sxx , syy, sxy = sum([i ** 2 for i in xx]), sum([i ** 2 for i in yy]), sum(xy)
    b = sxy / sxx
    a = mean(ys) - b * mean(xs)
...
def rate_all(user, restaurants, feature_fns):
    ...
    reviewed = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 9
    return {restaurant_name(i): predictor(i) if i not in reviewed else user_rating(user,restaurant_name(i)) for i in restaurants}
    # END Question 9
