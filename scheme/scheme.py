...
def make_call_frame(procedure, args, env):
    ...
    # BEGIN Question 12
    if type(procedure) == LambdaProcedure:
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError
        def helper(formals, vals, count):
            if count >= 1:
                child.define(formals.first, vals.first)
                return helper(formals.second, vals.second, count-1)
        helper(formals, vals, len(formals))
        # END Question 10
        return child
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        lambda_vals = Pair(target.second, expressions.second)
        lambda_func = do_lambda_form(lambda_vals, env)
...
def do_lambda_form(expressions, env):
    ...
    if len(body) != 1:
        return LambdaProcedure(formals, Pair(body.first, body.second), env)
...
def do_and_form(expressions, env):
    ...
    if len(expressions) == 0:
        return True
    def helper(expressions, count):
        if count > 1:
            if not scheme_true(scheme_eval(expressions.first, env)):
                return False
            else:
                return helper(expressions.second, count-1)
        else:
            return scheme_eval(expressions.first, env)
    return helper(expressions, len(expressions))
    # END Question 14B
...
def make_let_frame(bindings, env):
    ...
    def helper(bindings, count):
        ...
        if scheme_symbolp(bindings.first.first):
            ...
            raise SchemeError
        if count > 1:
            return helper(bindings.second, count-1)
        else:
            return formals, values
    formals, values = helper(bindings, len(bindings))
...
def check_formals(formals):
    ...
    symbols = []
    while len(formals):
        if not scheme_symbolp(formals.first) or formals.first in symbols:
            raise SchemeError
        symbols.append(formals.first)
...
def do_mu_form(expressions, env):
    ...
    body = expressions.second
    if len(body) != 1:
        return MuProcedure(formals, Pair(body.first, body.second))
    return MuProcedure(formals, Pair(body.first, nil))
    # END Question 17
