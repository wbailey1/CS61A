...
(define hola 'hi)
; expect hola
(define greeting (cons hola (cons hi nil)))
; expect greeting
greeting
; expect (hi there)
...
;;; Q7
(begin (define x 4) (define y 5) (define z 6))
; expect z
x
; expect 4
y
...
; expect (lambda (x) (+ (+ (+ (+ (+ x x) x) x) x) x))
(define f (lambda (x) (+ (+ (+ (+ (+ x x) x) x) x) x)))
; expect f
f
; expect (lambda (x) (+ (+ (+ (+ (+ x x) x) x) x) x))
(f 4)
...
(if (= 3 4) 'yup 'nah)
; expect nah
(if (< 1 2) (if (> 1 2) 17 42) (if (> 42 17) 1 2))
; expect 42

;;; Q14
(or (and 0 (= 3 4) False) (and 12 16 (+ 4 38)) False)
; expect 42

;;; Q15
(define x '(3 2 5 7 1 2))
; expect x
(cond
  ((= (car x) 7) 'nah)
  ((> (car (cdr x)) 7) 'nah)
  ((< (car (cdr (cdr x))) 7) 'yup)
  ((null? (cdr (cdr (cdr x)))) 'nah)
  (else 'hithere)
)
; expect yup

;;; Q16
(let ((x 3) (y 4) (z 6)) (+ x (* y z)))
; expect 27
